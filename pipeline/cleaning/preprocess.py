...
def preprocess():
    ...
    # Simpan kolom plat dulu sebelum ffill
    plat_backup = df["plat"].copy()
    ...
    # Restore plat kalau hilang
    df["plat"] = plat_backup

    df["cumulative_km"] = df.groupby("plat")["jarak_km"].cumsum()

    for col in ["hard_brake", "suhu_mesin", "jarak_km"]:
        df[f"{col}_roll7"] = df.groupby("plat")[col].transform(
            lambda x: x.rolling(7, min_periods=1).mean())

    df["hard_brake_std7"] = df.groupby("plat")["hard_brake"].transform(
